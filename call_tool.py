...
class BamlStructuredAgent(Agent):
    ...
    def complete_question(self, acknolwedgement):
    ...
    async def llm_node(
        ...
        try:
            ...
            logger.info("Calling BAML GetResponse2...")
            structured: Response = await b.GetResponse2(chat_history=history_str)
            logger.info(f"BAML Response: {structured}")

            if structured.is_answer_satisfactory:
                self.complete_question(structured.acknowledgement)
            else:
                self.session.say(structured.probe_question)
...
async def entrypoint(ctx: JobContext):
    ...
    await session.say("Can you tell me your hobbies?")
